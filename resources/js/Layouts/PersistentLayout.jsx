...
import {
  Box, AppBar, Toolbar, Typography, IconButton, Menu, MenuItem, Avatar, useMediaQuery
} from '@mui/material';
...
<Box sx={{ display: 'flex', alignItems: 'center', gap: 1 }}>
  ...
  {!isMobile && (
    <Box sx={{ lineHeight: 1 }}>
      <Typography variant="body1" fontWeight="bold" color="#450001" sx={{ m: 0, p: 0 }}>
        {userData.name}
      </Typography>
      <Typography variant="body2" color="text.secondary" sx={{ fontSize: '0.75rem', m: 0, p: 0 }}>
        {userData.email}
      </Typography>
    </Box>
  )}
  <IconButton onClick={handleAvatarClick} sx={{ color: '#450001', p: 0 }}>
    <ArrowDropDownIcon />
  </IconButton>
